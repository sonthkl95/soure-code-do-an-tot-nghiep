...
# Static global IP for Grafana LB
...
# Health check on Grafana HTTP endpoint
...
# Backend service pointing to Grafana instance group (cross-project)
resource "google_compute_backend_service" "gcp-asia-southeast1-backend-grafana-001" {
  ...
  depends_on = [google_project_iam_member.lb_service_user_obs]
}

# URL map – all traffic goes to Grafana backend
...
# HTTP target proxy linking forwarding rule to URL map
...
# Global forwarding rule: static IP + port 80 -> Grafana proxy
